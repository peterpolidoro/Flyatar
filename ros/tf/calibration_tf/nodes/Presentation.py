...
class Calibration:
  ...
  def initialize_images(self,cv_image):
    ...
    self.im_distorted = cv.CreateImage(self.im_size,cv.IPL_DEPTH_8U,3)
    ...
    self.undistorted_points = cv.CreateMat(self.im_width*self.im_height,3,cv.CV_32FC1)
    self.distorted_points = cv.CreateMat(self.im_width*self.im_height,3,cv.CV_32FC1)
  ...
  def image_undistorted_callback(self,data):
    try:
    ...
    cv.CvtColor(cv_image,self.im_display,cv.CV_GRAY2BGR)

    if not self.undistorted_saved:
      try:
        self.undistorted_camera = self.tf_listener.transformPoint("UndistortedImage",self.camera_origin)
        cv.Circle(self.im_display, (int(self.undistorted_camera.point.x),int(self.undistorted_camera.point.y)), 5, cv.CV_RGB(self.color_max,0,self.color_max), cv.CV_FILLED)

        cv.SaveImage("undistorted.png",self.im_display)
        rospy.logwarn("Undistorted image saved!!!")
        self.undistorted_saved = True
    ...
    cv.ShowImage("Presentation", self.im_display)
    cv.WaitKey(3)

  def image_distorted_callback(self,data):
    try:
      cv_image = cv.GetImage(self.bridge.imgmsg_to_cv(data, "passthrough"))
    except CvBridgeError, e:
      print e

    if not self.distorted_saved:
      if self.images_initialized:
        cv.CvtColor(cv_image,self.im_distorted,cv.CV_GRAY2BGR)

        try:
          self.undistorted_camera = self.tf_listener.transformPoint("UndistortedImage",self.camera_origin)
          cv.Circle(self.im_distorted, (int(self.undistorted_camera.point.x),int(self.undistorted_camera.point.y)), 5, cv.CV_RGB(self.color_max,0,self.color_max), cv.CV_FILLED)

          cv.SaveImage("distorted.png",self.im_distorted)
          rospy.logwarn("Distorted image saved!!!")
          self.distorted_saved = True
        except (tf.LookupException, tf.ConnectivityException):
          pass
